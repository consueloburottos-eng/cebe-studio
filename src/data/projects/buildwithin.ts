...
export const buildwithinCandidatePortal: Project = {
  ...
  tag: "product design · candidate experience (individual end-user) · enterprise saas · ia",
  ...
  en: {
    ...
    brief:
      "BuildWithin — Candidate Portal is the full redesign of the candidate experience inside BuildWithin's transformation into an AI-powered Workforce platform. The original platform had been built for a different market; we evolved it so candidates, Talent Managers, Case Managers, Administrators, and Super Admins could all operate within the same ecosystem, each with completely different needs. The most critical process was eligibility validation: traditionally manual, it required candidates to upload multiple documents while administrators reviewed each one for category, expiration, signatures, profile match, and image quality — often re-reviewing the same document valid across several categories. As Senior Product Designer (Contractor) I led the design end to end: defined the candidate experience, designed the core flows, and continuously validated decisions alongside Product, the client, and BuildWithin's CEO, aiming to automate document validation, cut review time, avoid duplicates, and improve the candidate experience without losing the platform's ability to scale across different Workforce organizations.",
    strategy: [
