...
const baseProjects: Project[] = [
  ...
  },
  {
    ...
    coverMedia: img("altafid", "08.jpg"),
    brief:
      "Diseñé el marketplace de Altafid para que los asesores no dependieran solo de los modelos construidos internamente en Model Builder: una capa conectada que les da acceso a modelos de inversión de gestoras externas, contenido curado sobre esos modelos y productos, y aplicaciones de terceros del ecosistema financiero, todo dentro de la misma plataforma.",
    strategy: [
      "El Marketplace de Modelos de Inversión conecta con gestoras líderes y permite personalizar cada modelo por tenencias y restricciones específicas del cliente, combinando estrategias activas y pasivas — alimentando directamente el mismo flujo de Model Builder y Portfolio Design en lugar de vivir como un catálogo aislado. El Marketplace de Contenido complementa esos modelos con información curada de terceros sobre productos y estrategias, con control de qué se comparte con equipos y con clientes.",
    ...
    en: {
      ...
      brief:
        "I designed Altafid's marketplace so advisors weren't limited to models built in-house through Model Builder: a connected layer giving them access to investment models from external asset managers, curated content about those models and products, and third-party apps from the financial ecosystem, all inside the same platform.",
      strategy: [
        "The Investment Models Marketplace connects to leading asset managers and lets advisors customize each model by client-specific holdings and restrictions, combining active and passive strategies — feeding directly into the same Model Builder and Portfolio Design flow instead of living as an isolated catalogue. The Content Marketplace complements those models with curated third-party information on products and strategies, with control over what gets shared with teams versus clients.",
        "The External Apps Marketplace solved a different problem: instead of every new integration requiring custom development, we designed a way for each entity to choose and bring in tools from their own stack (CRM, reporting, communications) without depending on a full development cycle, letting each firm's team decide which apps to activate based on their needs.",
      ],
