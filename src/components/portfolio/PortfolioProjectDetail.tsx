...
export default function PortfolioProjectDetail({ project: rawProject, others: rawOthers }: { project: Project; others: Project[] }) {
  ...
  const others = localizeProjects(rawOthers.slice(0, 3), lang);
      ...
      <div className="w-full flex-1 px-4 py-14 sm:px-8 sm:py-20">
        ...
        {others.length > 0 && (
          <section className="mt-24 border-t pt-16 sm:mt-32 sm:pt-20" style={{ borderColor: "var(--cb-hair)" }}>
            <h2 className="m-0 mb-10 text-[clamp(18px,2.2vw,22px)] sm:mb-14" style={serif}>
              {lang === "en" ? "More projects" : "Más proyectos"}
            </h2>
            <div className="grid grid-cols-1 gap-8 sm:grid-cols-3">
              {others.map((other) => (
                <Link key={other.slug} href={`/portfolio/projects/${other.slug}`} className="group flex flex-col gap-3 no-underline">
                  <div className="relative aspect-[4/3] w-full overflow-hidden" style={{ background: "var(--cb-pill)" }}>
                    <ProjectMedia
                      media={other.coverMedia}
                      label={other.cover}
                      sizes="(min-width:1100px) 360px, 33vw"
